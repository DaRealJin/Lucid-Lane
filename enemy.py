...
class Zombie(Sprite):
    #Enemy NPC with simple state-based AI: idle->chase->return
    def __init__(self, x, y):
        ...
        self.spawn_py = y
        self.aggro_radius = 8
        #Within this many tiles the zombie switches to chase mode
        self.state = "idle"
    ...
    def update(self, grid, tilemap, player):
        #Runs every frame; uses a think timer so decisions don't change every single frame
    ...
    def _walkable(self, grid, tilemap, tx, ty):
        #Prevents zombies walking through solid tiles and outside the map bounds(self, grid, tilemap, tx, ty):
